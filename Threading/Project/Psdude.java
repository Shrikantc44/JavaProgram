...
public class Psdude extends JPanel implements Runnable {
    ...
    private void clockLayout(Graphics g) {
        ...
        g.drawString("CALENDAR", 670, 255);
        
        // Date boxes
        g.setColor(Color.black);
        g.fillRect(620, 280, 80, 50);
        g.fillRect(710, 280, 80, 50);
        g.fillRect(800, 280, 80, 50);
        ...
        g.drawString("MM", 720, 310);
        g.drawString("YYYY", 810, 310);
        
        // Date values
        g.drawString(day, 645, 385);
        g.drawString(month, 735, 385);
        g.drawString(year, 830, 385);
		
		// Date values
       // g.drawString(day.length() == 1 ? "0"+day : day, 630, 350);  // DD with leading zero
       // g.drawString(month.length() == 1 ? "0"+month : month, 720, 350); // MM with leading zero
       // g.drawString(year, 810, 350);  // YYYY
        
        // Time values
        g.drawString(hour + "H", 645, 505);
        g.drawString(minute + "M", 735, 505);
        ...
        int mainTitleWidth = g.getFontMetrics().stringWidth("Clock Design By Shrikant Chauhan");
        int mainTitleX = (getWidth() - mainTitleWidth) / 2;
        g.drawString("Clock Design By Shrikant Chauhan", mainTitleX, 100);
		// Calculate position for subtitle
         int subtitleWidth = g.getFontMetrics().stringWidth("Under the Guidance of");
         int subtitleX = (getWidth() - subtitleWidth) / 2;
         g.drawString("Under the Guidance of", subtitleX, 150);
		 
		 // Teacher's name with larger font
         g.setColor(Color.black);
         g.setFont(new Font("Arial", Font.BOLD, 35));
         int teacherWidth = g.getFontMetrics().stringWidth("Mr. Manish Bhatia Sir");
         int teacherX = (getWidth() - teacherWidth) / 2;
         g.drawString("Mr. Manish Bhatia Sir", teacherX, 200);
    ...
    public void paint(Graphics g) {
        ...
        int str1[] = {0, 10, 20, 30, 40, 50, 58, 64, 76, 84, 88, 91, 95, 97, 99, 100};
        int str2[] = {0, 40, 69, 80};
